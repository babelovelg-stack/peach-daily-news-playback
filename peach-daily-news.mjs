...
function buildNewsQuizQuestions(news, dayIndex = 0) {
  ...
  if (/玉净虫/.test(newsText) && /化石|附肢/.test(newsText)) {
    add(
      "fossil-preservation-missing-evidence-1",
      ["化石", "古生物", "证据"],
      "探究题：研究玉净虫化石时，怎样区分“原来没有某种附肢”和“附肢没有保存下来”？假设后续找到的一批化石有的清楚、有的受到挤压，这是假设情境，不是报道中的四件标本记录。哪组资料最能帮助判断？\nA. 汇总这批标本中发现附肢的数量，计算它占全部标本的比例，再与另一种古生物的发现比例比较\nB. 依据其他细小结构的保存情况给标本分组，再比较各组出现附肢的比例，并核对不同埋藏条件下的标本\nC. 测量这批标本的身体长度，按大中小分组，再比较各组出现附肢的比例，检查附肢是否与个体大小有关",
      "答案：B。没有观察到，不等于原来不存在。附肢可能在埋藏、挤压和形成化石的过程中损坏。B用其他细小结构的保存情况评估标本保存质量，再比较附肢出现的比例，能检查缺失是否伴随保存不佳；核对不同埋藏条件还能增加线索。A混合了保存质量不同的标本，另一种动物也可能有不同的保存机会；C能研究大小与附肢的关系，却仍未区分保存质量。即使保存良好的标本也没有附肢，仍需更多证据，不能仅凭缺失就认定原来没有。",
      4,
      "comparative-evidence"
    );
  }

  if (/桃果实|桃果肉|红肉桃/.test(newsText) && /花青素|Rbl/.test(newsText)) {
    add(
      "peach-pigment-light-interaction-1",
      ["花青素", "植物", "数据"],
      "资料题：研究人员想比较，光照增加花青素的幅度是否会受Rbl状态影响。以下是为了练习而设定的模拟数据，不是新闻中的实测数值：Rbl正常组，遮光时色素指数为2、光照时为4；Rbl受抑组，遮光时为3、光照时为11。其他实验条件相同。哪种分析最能回答这个问题？\nA. 比较两组光照时的指数，受抑组比正常组高7，认为光照带来的增量在两组之间相差7\nB. 分别计算各组光照比遮光增加多少，正常组增加2、受抑组增加8，再比较两个增量相差6\nC. 计算各组光照和遮光的平均指数，正常组为3、受抑组为7，认为光照带来的增量相差4",
      "答案：B。要比较光照带来的增量，先在同一种Rbl状态内，用光照结果减去遮光结果：4－2＝2，11－3＝8；再比较两个增量，8－2＝6。A把光照时的结果相减，混入了遮光时已经存在的差别；C把两种光照条件取平均，得到的是平均水平，不是光照带来的增量。模拟数据支持两组的光照反应幅度不同，但不能代替真实研究对分子机制的验证。",
      4,
